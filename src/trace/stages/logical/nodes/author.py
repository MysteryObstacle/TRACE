...
def author_node(state: LogicalState, role_client) -> LogicalState:
    author_tools = LogicalAuthorTools(
        state=state,
        logical_constraints=_logical_constraint_ids(state),
    )
    messages = build_messages(
        system_prompt=PROMPT_PATH.read_text(encoding="utf-8").strip(),
        task="Author logical-stage checkpoint functions for the current topology problem.",
        context_sections=_logical_author_context(state),
...
class LogicalAuthorTools:
    def __init__(self, *, state: LogicalState, logical_constraints: list[dict[str, Any]]) -> None:
        self._state = state
        self._known_constraint_ids = {
            str(item.get("id") or "").strip()
            for item in logical_constraints
    ...
    def write_checkpoint_file(self, *, content: str, path: str = DEFAULT_CHECKPOINT_PATH) -> dict[str, Any]:
        ...
        self._checkpoint_files["logical"] = normalized_path
...
def _logical_author_context(state: LogicalState) -> dict[str, Any]:
    ground_artifact = state.get("ground_artifact", {})
    context: dict[str, Any] = {
        "node_groups": ground_artifact.get("node_groups", []),
        "constraint_files": state.get("draft_artifact", {}).get("constraint_files", {})
        or ground_artifact.get("constraint_files", {"logical": DEFAULT_CONSTRAINT_PATH}),
    }
    return context


def _logical_constraint_ids(state: LogicalState) -> list[dict[str, Any]]:
    path = (
        state.get("draft_artifact", {}).get("constraint_files", {}).get("logical")
        or state.get("ground_artifact", {}).get("constraint_files", {}).get("logical")
