...
def repair_node(state: LogicalState, role_client) -> LogicalState:
    ...
    authored_checkpoints = state["draft_artifact"].get("logical_checkpoints", [])
    authored_script = state["draft_artifact"].get("logical_validator_script")
    bound_tools = BoundTGraphTools.from_json(
        state["draft_artifact"]["tgraph_logical"],
        ...
        constraints=state.get("ground_artifact", {}).get("logical_constraints", []),
    )
    ...
    post_repair_report = bound_tools.validate()
    ...
    state["events"] = [*state.get("events", []), {"type": "logical.repair.completed", "attempt": state["attempt"]}]
    return state
...
def _build_repair_messages(
    *,
    ...
    logical_validator_script: str | None,
) -> list[dict[str, str]]:
    messages: list[dict[str, str]] = [
        ...
        {
            "role": "human",
            "content": "Use the provided node/link TGraph tools to repair the logical artifact.",
        },
        {
            "role": "human",
        ...
        {
            "role": "human",
            "content": _format_section("logical_constraints", logical_constraints),
        },
        {
            "role": "human",
    ...
    if logical_validator_script:
        messages.append(
            {
                "role": "human",
                "content": _format_section("logical_validator_script", logical_validator_script),
            }
        )
    return messages
