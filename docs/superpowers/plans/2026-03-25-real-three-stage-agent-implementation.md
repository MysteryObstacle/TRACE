...
**Status:** Completed on 2026-03-25. A few files listed in the original file map did not require direct edits because adjacent runtime/tests already covered the intended behavior.
...
def test_ground_guard_rejects_unknown_compact_refs() -> None:
    output = GroundOutput(
        ...
        logical_constraints=[{'id': 'lc1', 'scope': 'node_ids', 'text': 'PLC[1..3] �������'}],
...
- [x] **Step 2: Run broader runtime suites**
...
- [x] **Step 3: Run the full test suite if the focused suites are green**

Run: `pytest -v`
Expected: PASS; if unrelated failures remain, capture them explicitly before any final summary.

- [x] **Step 4: Update README language to describe the new runtime honestly**
