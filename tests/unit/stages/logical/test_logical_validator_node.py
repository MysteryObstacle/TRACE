...
def test_logical_validator_routes_escalation_issues_to_repair_not_escalate(tmp_path) -> None:
    state = {
        "draft_artifact": {
            "constraint_files": {"logical": "ground/logical_constraints.json"},
            "checkpoint_files": {"logical": "logical/checkpoints.py"},
            "graph": {"stage": "logical", "nodes": [{"id": "A", "type": "router", "label": "A", "ports": []}], "links": []},
        },
        "support_files": {
            "ground/logical_constraints.json": '{"lc1": {"kind": "logical.custom", "statement": "conflicting fact"}}',
            "logical/checkpoints.py": (
                "def check_lc1(tgraph):\n"
                "    return tgraph.escalate('logical.escalation.constraint_conflict', 'conflict')\n"
            ),
        },
        "support_file_root": str(tmp_path),
        "attempt": 1,
    ...
    assert result.goto == "repair"
    assert result.update["evaluation_report"]["issues"][0]["details"]["issue_kind"] == "logical.escalation.constraint_conflict"
